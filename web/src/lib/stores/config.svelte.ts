...
class ConfigStore {
  ...
  // Sets `error` on failure and re-throws so callers can react (e.g.
  // revert local state on auto-apply, surface a retry affordance, etc.).
  // The previous fire-and-forget swallow made the caller indistinguishable
  // from success, which is fine for a global error chip but unworkable
  // for per-control optimistic UI.
