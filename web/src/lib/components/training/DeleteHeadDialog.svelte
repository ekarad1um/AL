...
<Modal {open} title="Delete this head?" {onclose} closeOnBackdrop={!submitting}>
  <!-- Identity strip: head id (mono) + classes count + size.
       Mirrors the workspace-delete dialog's identity strip so
       the two destructive surfaces feel like a family. -->
  {#if head}
    <div
      class="rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2 text-xs text-zinc-900 wrap-break-word"
    >
      <p class="font-mono text-[10px]" title={head.head_id}>{head.head_id}</p>
