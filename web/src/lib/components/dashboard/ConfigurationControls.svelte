<script lang="ts">
  ...
  // Microphone + Inference cadence form body, shared verbatim by the
  // dashboard's `ConfigurationPanel` and the workspace `DeployPane`'s
  // collapsed-by-default disclosure.  Owns its own apply-floor +
  // optimistic-revert behaviour so both surfaces stay in lockstep
  // without duplicating the form-state machine.  The outer section
  // (border + padding + title) is the host surface's responsibility.
...
{#if unavailable}
  <!-- +layout.svelte retries config.refresh on health-level recovery;
       this body just shows a clear "waiting" message until then. -->
